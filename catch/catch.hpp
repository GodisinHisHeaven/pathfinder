...
namespace detail {
    ...
    template<typename T>
    inline auto convertInto( std::string const &dataset, T& target ) -> ParserResult {
        std::stringstream ss;
        ss << dataset;
        ss >> target;
        if( ss.fail() )
            return ParserResult::runtimeError( "Unable to convert '" + dataset + "' to destination type" );
        else
            return ParserResult::ok( ParseResultType::Matched );
    }
    inline auto convertInto( std::string const &dataset, std::string& target ) -> ParserResult {
        target = dataset;
        return ParserResult::ok( ParseResultType::Matched );
    }
    inline auto convertInto( std::string const &dataset, bool &target ) -> ParserResult {
        std::string srcLC = dataset;
        ...
        if (srcLC == "y" || srcLC == "1" || srcLC == "true" || srcLC == "yes" || srcLC == "on")
            ...
            return ParserResult::runtimeError( "Expected a boolean value but did not recognise: '" + dataset + "'" );
        return ParserResult::ok( ParseResultType::Matched );
    }
#ifdef CLARA_CONFIG_OPTIONAL_TYPE
    template<typename T>
    inline auto convertInto( std::string const &dataset, CLARA_CONFIG_OPTIONAL_TYPE<T>& target ) -> ParserResult {
        T temp;
        auto result = convertInto( dataset, temp );
        if( result )
...
namespace Matchers {
    ...
    namespace StdString {
        ...
        bool EqualsMatcher::match( std::string const& dataset ) const {
            return m_comparator.adjustString( dataset ) == m_comparator.m_str;
        }

        ContainsMatcher::ContainsMatcher( CasedString const& comparator ) : StringMatcherBase( "contains", comparator ) {}

        bool ContainsMatcher::match( std::string const& dataset ) const {
            return contains( m_comparator.adjustString( dataset ), m_comparator.m_str );
        }

        StartsWithMatcher::StartsWithMatcher( CasedString const& comparator ) : StringMatcherBase( "starts with", comparator ) {}

        bool StartsWithMatcher::match( std::string const& dataset ) const {
            return startsWith( m_comparator.adjustString( dataset ), m_comparator.m_str );
        }

        EndsWithMatcher::EndsWithMatcher( CasedString const& comparator ) : StringMatcherBase( "ends with", comparator ) {}

        bool EndsWithMatcher::match( std::string const& dataset ) const {
            return endsWith( m_comparator.adjustString( dataset ), m_comparator.m_str );
        }
